...
export default function PageClient() {
  ...
  const questions = [
    ...
    data.sansu[
      seed % data.sansu.length
    ],

    data.sansu[
      (seed + 1) % data.sansu.length
    ],

    data.ronri[
      seed % data.ronri.length
    ],

    data.ronri[
      (seed + 1) % data.ronri.length
    ],

    data.pattern[
      seed % data.pattern.length
    ],

    data.pattern[
      (seed + 1) % data.pattern.length
    ],

    data.hiragana[
      seed % data.hiragana.length
    ],

    data.hiragana[
      (seed + 1) % data.hiragana.length
    ],

    data.nakamawake[
      seed % data.nakamawake.length
    ],

    data.nakamawake[
      (seed + 1) % data.nakamawake.length
    ],
